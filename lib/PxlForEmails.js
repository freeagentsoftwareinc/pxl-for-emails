    ...
    addTracking(htmlEmail, metadata) {
        ...
        return this.addOpenTracking(htmlEmail, metadata, true)
            .spread((updatedHtmlEmail, openTrackingPxl) => {
                return this.addClickTracking(updatedHtmlEmail, metadata, true, openTrackingPxl.pxl)
                .then( (html)=> ({ html, pxl: openTrackingPxl }));
            })
        ...
        return BPromise.try(() => {
            ...
            return stringReplaceAsync.seq(
                ...
                BPromise.coroutine(function *(match, p1, link, p3) {
                    ...
                    let newLink = `${ p1 }${ this.options.openTracking.apply(linkToUse, openPxl.pxl) }${ p3 }` // eslint-disable-line prefer-reflect
